...
export default class AsciiMathPlugin extends Plugin {
  ...
  async onload() {
    ...
    // used when changing settings later
    this.previousSettings = this.settings.blockPrefix;
  ...
  editorTransactionConvertFormula(editor: Editor) {
    ...
    try {
      ...
      new Notice(
        `Successfully converted ${changes.length} asciimath formulas!`
      );
    } catch (e) {
      new Notice(String(e));
    }
      ...
      .addButton((btn) =>
        btn.setButtonText("Save").onClick(async () => {
          ...
          this.plugin.settings.blockPrefix.forEach((prefix) => {
            if (!this.plugin.previousSettings.includes(prefix)) {
